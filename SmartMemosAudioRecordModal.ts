...
export class SmartMemosAudioRecordModal extends Modal {
  ...
  private startTime = 0;
  private elapsedTime = 0; // To keep track of the elapsed time
  private redDot: HTMLElement;
  private isResetting = false; // Flag to track reset state
  ...
  onOpen() {
    ...
    const headerTimerContainer = contentEl.createDiv({
      cls: 'smart-memo-header-timer-container',
    });
    const header = headerTimerContainer.createEl('h2', {
      text: 'Recording...',
      cls: 'smart-memo-recording-header',
    });
    this.timer = headerTimerContainer.createEl('div', {
      cls: 'smart-memo-timer',
      text: '00:00',
    });
    ...
    const redDotContainer = contentEl.createDiv({
      cls: 'smart-memo-red-dot-container',
    });
    this.redDot = redDotContainer.createDiv({ cls: 'smart-memo-red-dot' });

    // Control buttons group
    const controlGroupWrapper = contentEl.createDiv({
      cls: 'smart-memo-control-group-wrapper',
    });
    const controlGroup = controlGroupWrapper.createDiv({
      cls: 'smart-memo-modal-button-group',
    });
    const playPauseButton = controlGroup.createEl('button', {
      cls: 'smart-memo-modal-button smart-memo-flex',
    });
    const stopButton = controlGroup.createEl('button', {
      cls: 'smart-memo-modal-button smart-memo-flex',
    });
    ...
    stopButton.addEventListener('click', async () => {
      ...
      this.handleAudioRecording(
        audioFile,
        false,
        (this.keepAudioCheckbox as HTMLInputElement).checked,
        (this.includeAudioFileLinkCheckbox as HTMLInputElement).checked,
      );
    });
    ...
    const transcribeButton = controlGroupWrapper.createEl('button', {
      cls: 'smart-memo-modal-button smart-memo-full-width-button smart-memo-transcribe-button',
    });
    ...
    const resetButton = contentEl.createEl('button', {
      cls: 'smart-memo-modal-button smart-memo-full-width-button smart-memo-reset-button',
      text: 'Restart',
    });
    ...
    // Add the checkbox
    const keepAudioContainer = contentEl.createDiv({
      cls: 'smart-memo-keep-audio-container',
    });
    this.keepAudioCheckbox = keepAudioContainer.createEl('input', {
      type: 'checkbox',
      cls: 'smart-memo-keep-audio-checkbox',
    });
    (this.keepAudioCheckbox as HTMLInputElement).checked =
      this.settings.keepAudio; // Set checked based on settings;
    const keepAudioLabel = keepAudioContainer.createEl('label', {
      text: 'Keep Audio File',
      cls: 'smart-memo-keep-audio-label',
    });
    keepAudioLabel.htmlFor = this.keepAudioCheckbox.id;

    // Add the checkbox for including audio file link
    const includeAudioFileLinkContainer = contentEl.createDiv({
      cls: 'smart-memo-include-audio-file-link-container',
    });
    this.includeAudioFileLinkCheckbox = includeAudioFileLinkContainer.createEl(
      'input',
      {
        type: 'checkbox',
        cls: 'smart-memo-include-audio-file-link-checkbox',
      },
    );
    (this.includeAudioFileLinkCheckbox as HTMLInputElement).checked =
      this.settings.includeAudioFileLink; // Set checked based on settings
    const includeAudioFileLinkLabel = includeAudioFileLinkContainer.createEl(
      'label',
      {
        text: 'Include Audio File Player',
        cls: 'smart-memo-include-audio-file-link-label',
      },
    );
